...
export const Mutation = {
    ...
    try {
      ...
      if (!cliente) {
        throw new GraphQLError(
          `No se encontró un cliente con ID ${args.clienteId}`,
          {
            extensions: { code: "NOT_FOUND" },
          },
        );
      ...
      if (!tarjeta) {
        throw new GraphQLError(
          `No se encontró una tarjeta con ID ${args.tarjetaId}`,
          {
            extensions: { code: "NOT_FOUND" },
          },
        );
  ...
  marcarViajeComoTerminado: async (
    ...
    try {
      ...
      if (!viaje) {
        throw new GraphQLError(
          `No se encontró un viaje con ID ${args.viajeId}`,
          {
            extensions: { code: "NOT_FOUND" },
          },
        );
  ...
  eliminarTarjetaDeCliente: async (
    _: unknown,
    args: { clienteId: string; tarjetaId: string },
  ): Promise<ClienteModelType> => {
    try {
      ...
      const tarjetaIndex = cliente.cards.findIndex((tarjeta) =>
        tarjeta.id === args.tarjetaId
      );
      if (tarjetaIndex === -1) {
        throw new GraphQLError(
          `No se encontró una tarjeta con ID ${args.tarjetaId} en el cliente`,
          {
            extensions: { code: "NOT_FOUND" },
          },
        );
      }
      await TarjetaModel.findByIdAndDelete(args.tarjetaId);
      await cliente.save();
      return cliente;
    } catch (error) {
      throw new GraphQLError(error.message, {
